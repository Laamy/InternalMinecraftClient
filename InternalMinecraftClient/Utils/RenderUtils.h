...
public:
	...
	void Draw(Vector2 position, Vector2 size, _RGB colour) {
		...
		ctx->fillRectangle(Vector4(position.x, size.x, position.y, size.y), colour, colour.a);
	};
	void DrawOutline(Vector2 position, Vector2 size, _RGB colour, float width) {
		if (ctx == nullptr) return;
		ctx->drawRectangle(Vector4(position.x, size.x, position.y, size.y), colour, colour.a, width);
	};
};
